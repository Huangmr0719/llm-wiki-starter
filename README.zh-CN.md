...
# 2. 选择语言（中文用户运行此命令）
./setup.sh
# 输入 2 选择中文 → CLAUDE.md + skills 全部切换为中文

# 3. 在 Obsidian 中打开此文件夹作为 vault
...
> 💡 **英文用户**：跳过 `./setup.sh`，根目录文件默认为英文版。详见 [README.md](README.md)。
...
│
├── skills/                 # ④ 技能 — 可复用的工作流
│   ├── ingest.md           #   摄入新资料（读→讨论→摘要→实体→索引）
│   ├── lint.md             #   健康检查（矛盾/过时/孤儿/去重/剔除/空白）
│   └── query-and-keep.md   #   查询归档（问→答→归档回 wiki）
│
├── i18n/                   # ⑤ 多语言 — 中英文翻译文件
│   └── zh-CN/              #   中文版 CLAUDE.md + skills
│       ├── CLAUDE.md
│       └── skills/
│
├── setup.sh                # ⑥ 语言选择脚本
...
Claude 会：读文章 → 和你讨论要点 → 创建摘要页 → 更新/创建实体页 → 更新主题综述 → 检查与已有知识的矛盾 → 更新 index → 追加 log。
...
用户 clone 仓库
    │
    ├─ 英文用户（默认）: 根目录文件直接是英文，无需任何操作
    │
    └─ 中文用户: 运行 ./setup.sh → 选择 2 → i18n/zh-CN/ 文件覆盖根目录
```

贡献翻译：在 `i18n/` 下新建 `ja/`、`ko/`、`fr/` 等目录，翻译 CLAUDE.md 和三个技能文件，然后在 `setup.sh` 中添加选项即可。
